...
<x-slot name="header">
        <h2 class=" text-white font-sans text-3xl leading-tight">
            {{ __('Roles') }}
                     ...
                     <x-jet-button class="" wire:click="RoleModal">Create</x-jet-button>
               ...
               @else
                       You have selected <strong>{{ count($checkedRoles) }}</strong> records, do you wan to select All <strong>{{$roles->total()}}</strong> records?
                       ...
                       <table class=" divide-y divide-slate-700  min-w-full leading-normal">
                                   ...
                                   @forelse ($roles as $role)
                                       ...
                                       <tr class="divide-y divide-slate-700">
                                            <td class="px-3 py-2  ">
                                           <x-jet-checkbox value="{{$role->id}}" wire:model="checkedRoles"></x-jet-checkbox>
                                           </td>
                                           ...
                                           <div class="flex items-center gap-x-3 justify-end">
                                               <x-button type="button" wire:click="EditRole({{ $role->id }})" icon="pencil" class="px-1.5 py-1"  primary />

                                               <x-button wire:click="DeleteConfirmaton({{ $role->id }})" icon="x"   class="px-1.5 py-1" negative />
                                           </div>
     ...
     <x-modal.card title="Roles" blur wire:model="showModalForm">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                @foreach($permissions as $permission)
                            ...
                            <x-checkbox id="right-label" label="{{$permission->name}}" wire:model.defer="checkedPermission" value="{{$permission->id}}"  />
                  ...
                  <div class="flex justify-between gap-x-4">
                      ...
                      <div class="flex">
                          <x-button flat label="Cancel" x-on:click="close" />
                           @if ($roleId)
                              <div class="flex items-center gap-x-3 justify-end">
                                <x-button wire:click="updateRole" label="Update" wire:loading.attr="disabled" primary />
                            </div>
                            @else
                            <div class="flex items-center gap-x-3 justify-end">
                                <x-button wire:click="createRole" label="Create" wire:loading.attr="disabled" primary />
                            </div>
                           @endif
