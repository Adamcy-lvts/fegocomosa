<div>
    @if (auth()->user()->hasRole(['Super-Admin', 'reader']))
        <x-slot name="header">
        ...
        <x-button label="Add Permission" wire:click="PermissionModal" icon="lock-closed" />
        ...
        <div class="dark:text-gray-400">
            ...
            @if ($checkPageItems)
                <div>
                    ...
                    @else
                        You have selected <strong>{{ count($checkedPermissions) }}</strong> records, do you wan to
                        select
                        All <strong>{{ $permissions->total() }}</strong> records?
                    ...
                    <tbody class="divide-y divide-slate-700">
                        @forelse ($permissions as $permission)
                            <tr class="divide-y divide-slate-700">
                                <td class="px-3 py-2  ">
                                    <x-jet-checkbox value="{{ $permission->id }}" wire:model="checkedPermissions">
                                    </x-jet-checkbox>
                                </td>
                                ...
                                <td class="px-3 py-1   text-sm">
                                    ...
                                    <div class="flex items-center gap-x-3 justify-end">
                                        <x-button type="button" wire:click="EditPermission({{ $permission->id }})"
                                            icon="pencil" class="px-1.5 py-1" primary />

                                        <x-button wire:click="DeleteConfirmaton({{ $permission->id }})" icon="x"
        ...
        <x-modal.card title="Permission" blur wire:model="showModalForm">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="col-span-1 sm:col-span-2">
                    <x-input label="Permission" placeholder="Add Permission" wire:model.defer="name" />
                </div>
            ...
            <x-slot name="footer">
                <div class="flex justify-between gap-x-4">
                    ...
                    <div class="flex">
                        ...
                        @if ($permissionId)
                            <div class="flex items-center gap-x-3 justify-end">
                                <x-button wire:click="updatePermission" label="Update" wire:loading.attr="disabled" />
                            </div>
                        @else
                            <div class="flex items-center gap-x-3 justify-end">
                                <x-button wire:click="createPermission" label="Create" wire:loading.attr="disabled" />
                            </div>
                        @endif
