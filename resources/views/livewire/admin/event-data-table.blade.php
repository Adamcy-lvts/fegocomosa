<div>
                ...
                <x-slot name="trigger">
                    ...
                    <span class="inline-flex rounded-md">
                        <button type="button"
                            ...
                            {{ count($checkedEvents) }} Selected Record(s)
    ...
    <div class="dark:text-gray-400">
        ...
        @if ($checkPageItems)
            <div>
                @if ($checkAllItems)
                    <div> You have selected all <strong>{{ $events->total() }}</strong> records</div>
                @else
                    You have selected <strong>{{ count($checkedEvents) }}</strong> records, do you wan to select All
                    <strong>{{ $events->total() }}</strong> records?
                ...
                <tbody class="divide-y divide-slate-700">
                    @forelse ($events as $event)
                        <tr class="divide-y divide-slate-700">
                            <td class="px-3 py-2  ">
                                <x-jet-checkbox value="{{ $event->id }}" wire:model="checkedEvents"></x-jet-checkbox>
                            </td>
                            <td class="px-3 py-2  text-gray-300  text-sm">
                                {{ $event->id }}
                            </td>
                            <td class="px-3 py-2   text-sm">
                                <p class="text-gray-300 whitespace-no-wrap">{{ $event->title }}</p>
                            </td>
                            ...
                            <td class="px-3 py-2   text-sm">
                                <p class="text-gray-300 whitespace-no-wrap">{{ $event->event_date }}</p>
                            </td>

                            <td class="px-3 py-2   text-sm">
                                <p class="text-gray-300 whitespace-no-wrap">{{ $event->venue }}</p>
                            </td>
                            ...
                            <td class="px-3 py-1   text-sm">
                                ...
                                <div class="flex items-center gap-x-3 justify-end">
                                    <x-button href="{{ route('event.edit', $event->slug) }}" type="button"
                                        icon="pencil" class="px-1.5 py-1" primary />

                                    <x-button wire:click="DeleteConfirmaton({{ $event->id }})" icon="x"
