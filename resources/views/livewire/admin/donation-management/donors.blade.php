...
<x-slot name="header">
 ...
 <x-jet-button wire:click="DonorModal">Create</x-jet-button>
...
<div class="my-2 flex sm:flex-row flex-col">
            ...
            <div class="col-span-1 md:col-span-2 md:grid md:grid-cols-3 md:gap-5">
               ...
               @else
                       You have selected <strong>{{ count($checkedDonors) }}</strong> records, do you wan to select All <strong>{{$donors->total()}}</strong> records?
                        ...
                        <table class=" divide-y divide-slate-700  min-w-full leading-normal">
							...
							@forelse ($donors as $donor)
                                ...
                                <tr class="divide-y divide-slate-700">
                                    <td class="px-3 py-2  ">
                                        <x-jet-checkbox value="{{$donor->id}}" wire:model="checkedDonors"></x-jet-checkbox>
                                    </td>
									...
									<td class="px-5 py-5 border-b border-gray-200 bg-gray-900 text-sm">
                                           ...
                                           <div class="flex items-center gap-x-3 justify-end">
                                                <x-button type="button" wire:click="editDonor({{ $donor->id }})" icon="pencil" class="px-1.5 py-1"  primary />

                                                <x-button wire:click="DeleteConfirmaton({{ $donor->id }})" icon="x"   class="px-1.5 py-1" negative />
                                          </div>
                                    </td>
                      ...
                      <div class="flex">
                          <x-button flat label="Cancel" x-on:click="close" />
                           @if ($donorId)
                            ...
                            @else
                            <div class="flex items-center gap-x-3 justify-end">
                                <x-button wire:click="Donate" label="Create" wire:loading.attr="disabled" primary />
                            </div>
                           @endif
