...
<x-slot name="header">
        <h2 class=" text-white font-sans text-3xl leading-tight">
            {{ __('Organizers') }}
                     ...
                     <x-jet-button class="" wire:click="OrgModal">Create</x-jet-button>
               ...
               @else
                       You have selected <strong>{{ count($checkedOrganizers) }}</strong> records, do you wan to select All <strong>{{$organisations->total()}}</strong> records?
                       ...
                       <table class=" divide-y divide-slate-700  min-w-full leading-normal">
                                   ...
                                   @forelse ($organisations as $organisation)
                                       ...
                                       <tr class="divide-y divide-slate-700">
                                            <td class="px-3 py-2  ">
                                           <x-jet-checkbox value="{{$organisation->id}}" wire:model="checkedOrganizers"></x-jet-checkbox>
                                           </td>
                                           ...
                                           <div class="flex items-center gap-x-3 justify-end">
                                               <x-button type="button" wire:click="showEditModal({{ $organisation->id }})" icon="pencil" class="px-1.5 py-1"  primary />

                                               <x-button wire:click="DeleteConfirmaton({{ $organisation->id }})" icon="x"   class="px-1.5 py-1" negative />
                                           </div>
                  ...
                  <div class="flex justify-between gap-x-4">
                      ...
                      <div class="flex">
                          <x-button flat label="Cancel" x-on:click="close" />
                           @if ($orgId)
                              <div class="flex items-center gap-x-3 justify-end">
                                <x-button wire:click="updateOrg" label="Update" wire:loading.attr="disabled" primary />
                            </div>
                            @else
                            <div class="flex items-center gap-x-3 justify-end">
                                <x-button wire:click="storeOrg" label="Create" wire:loading.attr="disabled" primary />
                            </div>
                           @endif
