 <div>
     ...
     <x-button label="Add Donations" wire:click="DonationModal" />
     ...
     <div class="dark:text-gray-400">
         ...
         @if ($checkPageItems)
             <div>
                 ...
                 @else
                     You have selected <strong>{{ count($checkedDonations) }}</strong> records, do you wan to select All
                     <strong>{{ $donations->total() }}</strong> records?
                 ...
                 <tbody class="divide-y divide-slate-700">
                     @forelse ($donations as $donation)
                         <tr class="divide-y divide-slate-700">
                             <td class="px-3 py-2  ">
                                 <x-jet-checkbox value="{{ $donation->id }}" wire:model="checkedDonations">
                                 </x-jet-checkbox>
                             </td>
                             ...
                             <td class="px-5 py-5 border-b border-gray-200 bg-gray-900 text-sm">
                                 ...
                                 <div class="flex items-center gap-x-3 justify-end">
                                     <x-button type="button" wire:click="editDonation({{ $donation->id }})"
                                         icon="pencil" class="px-1.5 py-1" primary />

                                     <x-button wire:click="DeleteConfirmaton({{ $donation->id }})" icon="x"
                 ...
                 <x-native-select label="Campaign" placeholder="Campaign" wire:model.defer="selectedCampaign">
                     @foreach ($campaigns as $campaign)
                         <option value="{{ $campaign->id }}"
                             ...
                             {{ $campaign->campaign_title }}</option>
                     @endforeach
                 </x-native-select>
             </div>
         ...
         <x-slot name="footer">
             <div class="flex justify-between gap-x-4">
                 ...
                 <div class="flex">
                     ...
                     @else
                         <div class="flex items-center gap-x-3 justify-end">
                             <x-button wire:click="Donate" label="Donate" wire:loading.attr="disabled" />
                         </div>
                     @endif
