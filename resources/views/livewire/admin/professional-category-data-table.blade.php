...
<x-slot name="header">
        <h2 class=" text-white font-sans text-3xl leading-tight">
            {{ __('Projects') }}
                     ...
                     <x-jet-button class="" wire:click="createCategoryModal">Create</x-jet-button>
               ...
               @else
                       You have selected <strong>{{ count($checkedCategories) }}</strong> records, do you wan to select All <strong>{{$procategories->total()}}</strong> records?
                       ...
                       <table class=" divide-y divide-slate-700  min-w-full leading-normal">
                                   ...
                                   @forelse ($procategories as $procategory)
                                       ...
                                       <tr class="divide-y divide-slate-700">
                                            <td class="px-3 py-2  ">
                                           <x-jet-checkbox value="{{$procategory->id}}" wire:model="checkedCategories"></x-jet-checkbox>
                                           </td>
                                         ...
                                         <td class="px-5 py-5 border-b border-gray-200 bg-gray-900 text-sm">
                                                    ...
                                                    <div class="flex-shrink-0 w-10 h-10">
                                                        <img class="h-10 w-10 rounded-full object-cover"
                                                            src="{{ asset('storage/svg_icons/'.$procategory->svg_icon)  }}" alt="{{ Auth::user()->name }}"  />
                                                    </div>
                                                </div>
                                        </td>
                                           <td class="px-3 py-2   text-sm">
                                           ...
                                           <div class="flex items-center gap-x-3 justify-end">
                                               <x-button wire:click="showEditCategory({{ $procategory->id }})" icon="pencil" class="px-1.5 py-1"  primary />

                                               <x-button wire:click="DeleteConfirmaton({{ $procategory->id }})" icon="x"   class="px-1.5 py-1" negative />
                                           </div>
                   ...
                   <x-slot name="footer">
                           @if ($CategoryId)
                              <div class="flex items-center gap-x-3 justify-end">
                                ...
                                <x-button type="submit" wire:click="updateProCategory" label="Update" wire:loading.attr="disabled" primary />
                            </div>
                            @else
                            <div class="flex items-center gap-x-3 justify-end">
                                <x-button wire:click="CancelConfirmation" label="Cancel" flat />
                                <x-button type="submit" wire:click="storeProCategory" label="Create" wire:loading.attr="disabled" primary />
                            </div>
                           @endif
